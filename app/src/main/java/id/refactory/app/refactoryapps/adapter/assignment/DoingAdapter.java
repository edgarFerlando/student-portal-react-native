...
public class DoingAdapter extends RecyclerView.Adapter<DoingAdapter.MyViewHolder> {
    ...
    public DoingAdapter(List<DataAssignments> assignments, AssignmentFragment.FragmentListener listener)
    {
...
//    }
    ...
    @Override
    public DoingAdapter.MyViewHolder onCreateViewHolder(ViewGroup viewGroup, int i){
        View itemView = LayoutInflater.from(viewGroup.getContext()).inflate(R.layout.activity_assignments_rows,viewGroup, false);
        return new MyViewHolder(itemView);
    }

    @Override
    public void onBindViewHolder(DoingAdapter.MyViewHolder viewHolder, final int position){

        final DoingAdapter.MyViewHolder holder = viewHolder;
        ...
        String assignStatus = "DOING";

        if(project.matches(projects) && assignStatuses.matches(assignStatus)){
            holder.tv_status.setBackgroundResource(R.color.backgroundOrange);
